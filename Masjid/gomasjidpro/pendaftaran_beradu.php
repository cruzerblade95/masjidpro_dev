...
if(!empty($_FILES)){
    ...
    if(($ext == 'jpg') || ($ext == 'jpeg') || ($ext == 'png') || ($ext == 'gif')){
        ...
        echo '<div class="w3-center w3-red">Jenis imej mestilah jpg, jpeg, gif atau png.</div></br>';
    }
}
...
//RUNNING UPDATE IF EDITING
    ...
    if(!empty($_POST['number']) && !empty($_POST['type']) && !empty($_POST['price']) && !empty($_POST['description'])){
        ...
        $toEditID = $_GET['edit'];
        $sqlSelect = $db2->query("SELECT * FROM beradu_room WHERE id_room
            = '$toEditID' ");
        $row = mysqli_fetch_assoc($sqlSelect);

        if($row['room_image']==''){
            $query = $db2->query("UPDATE beradu_room SET `id_room`='$id_room',`room_type`='$room_type',`price`='$price',`room_image`='$room_image',`total_room`='$total_room' WHERE id_produk = '$toEditID' ");
        } else {
            $query = $db2->query("UPDATE beradu_room SET `id_room`='$id_room',`room_type`='$room_type',`price`='$price',`room_image`='$room_image',`total_room`='$total_room' WHERE id_produk = '$toEditID' ");
        }
        ...
        echo '<div class="w3-center w3-red">Sila isi semua ruangan.</div></br>';
    }
}
...
if(isset($_GET['delete_image'])){
    ...
    $imageURL = $_SERVER['DOCUMENT_ROOT'].'/ht/'.$fetch['room_image'];
    unlink($imageURL);
    ##################################################################
    $sql = "UPDATE beradu_room SET `room_image` = '' WHERE  = '$toEditID' ";
    $db2->query($sql);
...
<div class="w3-container w3-main" style="margin-left:200px">
    <header class="w3-container w3-purple">
        ...
        <h2 class="text-center">Tambah Bilik</h2>
    </header>
    <br/>
    <form class="form" action="#" method="post" enctype="multipart/form-data">
        ...
        <?php if(isset($_GET['edit']) && !empty($_GET['edit'])): ?>
            <div class="form-group col-md-4">
                <label></label>
                <a class="btn btn-block btn-danger btn-lg" href="rooms.php">Batal Kemaskini</a>
            </div>
