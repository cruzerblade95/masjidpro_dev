...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Senarai Ahli Kariah Perempuan Layak Mengundi ');
//setting Font Style
$styleFont = array(
	'font' => array(
		'bold'      => true,
...
$objPHPExcel->getActiveSheet()->setCellValue('E5', 'Tarikh Lahir');
$objPHPExcel->getActiveSheet()->setCellValue('F5', 'Jantina');
$objPHPExcel->getActiveSheet()->setCellValue('G5', 'No.Telefon');
...
//$currentYear= get_setting();
	...
	$sqlp="SELECT * FROM sej6x_data_peribadi WHERE id_masjid='$id_masjid' AND jantina='2' AND data_undi='1'";
	 $sqlqueryp = mysql_query($sqlp, $bd) or die ("Error :".mysql_error($bd));

$cnt=0;
$noindex = 5;
while($listp = mysql_fetch_assoc($sqlqueryp)){
		$cnt++;
		$noindex++;
		$objPHPExcel->getActiveSheet()->setCellValue("B$noindex", $cnt);
		$objPHPExcel->getActiveSheet()->setCellValue("C$noindex", $listp['nama_penuh']);
		$objPHPExcel->getActiveSheet()->setCellValue("D$noindex", $listp['no_ic']);
		$objPHPExcel->getActiveSheet()->setCellValue("E$noindex", $listp['tarikh_lahir']);
		$objPHPExcel->getActiveSheet()->setCellValue("F$noindex", 'Perempuan');
		$objPHPExcel->getActiveSheet()->setCellValue("G$noindex", $listp['no_hp']);
		$objPHPExcel->getActiveSheet()->setCellValue("H$noindex", $listp['alamat_terkini']);
...
$objWriter->save('output/'.$kod_masjid.'/SenaraiLayakMengundiPerempuan.xlsx');
