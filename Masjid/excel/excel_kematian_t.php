...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Senarai Kematian Tanggungan Kariah ');
//setting Font Style
$styleFont = array(
	'font' => array(
		'bold'      => true,
...
$objPHPExcel->getActiveSheet()->setCellValue('D5', 'No K/P');
$objPHPExcel->getActiveSheet()->setCellValue('E5', 'Tarikh Kematian');
$objPHPExcel->getActiveSheet()->setCellValue('F5', 'Tarikh Dikebumikan');
...
//$currentYear= get_setting();
	...
	$sql="SELECT b.nama_penuh,b.no_ic,a.tarikh_kematian,a.tarikh_dikebumikan,a.id_anak,a.id_kematian,b.ID,b.status_kematian
									FROM data_kematian a,sej6x_data_anakqariah b
									WHERE a.id_anak=b.ID AND b.status_kematian='1' AND b.id_masjid='$id_masjid'"; ;
...
while($list = mysql_fetch_assoc($sqlquery)){
		...
		$objPHPExcel->getActiveSheet()->setCellValue("E$noindex", $list['tarikh_kematian']);
		$objPHPExcel->getActiveSheet()->setCellValue("F$noindex", $list['tarikh_dikebumikan']);
...
$objWriter->save('output/'.$kod_masjid.'/SenaraiKematianTanggungan.xlsx');
