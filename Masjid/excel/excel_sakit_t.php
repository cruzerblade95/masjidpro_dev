...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Senarai Tanggungan Sakit Kronik');
//setting Font Style
$styleFont = array(
	'font' => array(
		'bold'      => true,
...
$objPHPExcel->getActiveSheet()->setCellValue('E5', 'Jenis Penyakit');
$objPHPExcel->getActiveSheet()->setCellValue('F5', 'Rawatan Terkini');
...
//$currentYear= get_setting();
	...
	$sql="SELECT a.nama_penuh, a.no_ic, b.jenis_penyakit, b.rawatan_terkini FROM sej6x_data_anakqariah a, sej6x_data_sakit b WHERE a.ID=b.id_anak AND a.id_masjid='$id_masjid' AND a.status_sakit='1'";
...
while($list = mysql_fetch_assoc($sqlquery)){
		...
		$objPHPExcel->getActiveSheet()->setCellValue("E$noindex", $list['jenis_penyakit']);
		$objPHPExcel->getActiveSheet()->setCellValue("F$noindex", $list['rawatan_terkini']);
...
$objWriter->save('output/'.$kod_masjid.'/SenaraiSakitKronikTanggungan.xlsx');
