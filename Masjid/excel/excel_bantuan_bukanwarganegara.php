...
$objPHPExcel->getActiveSheet()->setCellValue('D5', 'No Passport');
...
$sql_excel = "SELECT * FROM bantuan_zakat WHERE id_masjid='$id_masjid' AND status_bantuan=1 AND no_passport IS NOT NULL ORDER BY id_bantuan ASC";
...
while($list = mysqli_fetch_assoc($query_excel)){
		...
		if($list['id_data']==NULL){
			...
			$objPHPExcel->getActiveSheet()->setCellValue("D$noindex", $list['no_passport']);
...
$objWriter->save('output/'.$kod_masjid.'/RekodBantuanBukanWarganegara.xlsx');
