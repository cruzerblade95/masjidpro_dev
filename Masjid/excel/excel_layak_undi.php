...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Senarai Ahli Kariah Layak Mengundi');
//setting Font Style
$styleFont = array(
	'font' => array(
		'bold'      => true,
...
while($list = mysql_fetch_assoc($result2)){
		...
		$objPHPExcel->getActiveSheet()->setCellValue("E$noindex", $list['tarikh_lahir']);
		$objPHPExcel->getActiveSheet()->setCellValue("F$noindex", $jantina);
		$objPHPExcel->getActiveSheet()->setCellValue("G$noindex", $list['no_hp']);
		$objPHPExcel->getActiveSheet()->setCellValue("H$noindex", $list['alamat_terkini']);
...
$objWriter->save('output/'.$kod_masjid.'/SenaraiLayakMengundi.xlsx');
