...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Senarai Tanggungan Orang Kelainan Upaya (OKU)');
//setting Font Style
$styleFont = array(
	'font' => array(
		'bold'      => true,
...
$objPHPExcel->getActiveSheet()->setCellValue('F5', 'Kategori OKU');
...
//$currentYear= get_setting();
	...
	$sql="SELECT a.ID,a.nama_penuh,a.no_ic,a.umur,b.jenis_oku,b.keterangan,b.id_oku FROM sej6x_data_anakqariah a, sej6x_data_oku b WHERE a.ID=b.id_anak AND a.id_masjid='$id_masjid'";
...
while($list = mysql_fetch_assoc($sqlquery)){
		...
		$objPHPExcel->getActiveSheet()->setCellValue("E$noindex", $list['umur']);
		$objPHPExcel->getActiveSheet()->setCellValue("F$noindex", $list['jenis_oku']);
		$objPHPExcel->getActiveSheet()->setCellValue("G$noindex", $list['keterangan']);
...
$objWriter->save('output/'.$kod_masjid.'/SenaraiOKUTanggungan.xlsx');
