...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Senarai Inventori');
//setting Font Style
$styleFont = array(
	'font' => array(
		'bold'      => true,
...
$objPHPExcel->getActiveSheet()->setCellValue('C5', 'Nama Inventori');
$objPHPExcel->getActiveSheet()->setCellValue('D5', 'No Rujukan');
$objPHPExcel->getActiveSheet()->setCellValue('E5', 'Tarikh Belian');
$objPHPExcel->getActiveSheet()->setCellValue('F5', 'Kuantiti');
$objPHPExcel->getActiveSheet()->setCellValue('G5', 'Harga Belian');
$objPHPExcel->getActiveSheet()->setCellValue('H5', 'Amaun');
...
//$currentYear= get_setting();
	 ...
	 $result2 = mysql_query($sql_search, $bd) or die ("Error :".mysql_error($bd));
...
while($list = mysql_fetch_assoc($result2)){
		...
		$objPHPExcel->getActiveSheet()->setCellValue("C$noindex", $list['nama_inventori']);
		$objPHPExcel->getActiveSheet()->setCellValue("D$noindex", $list['no_rujukan']);
		$objPHPExcel->getActiveSheet()->setCellValue("E$noindex", $list['tarikh_belian']);
		$objPHPExcel->getActiveSheet()->setCellValue("F$noindex", $list['kuantiti']);
		$objPHPExcel->getActiveSheet()->setCellValue("G$noindex", $list['harga_belian']);
		$objPHPExcel->getActiveSheet()->setCellValue("H$noindex", $list['amaun']);
...
$objWriter->save('output/'.$kod_masjid.'/SenaraiInventori.xlsx');
