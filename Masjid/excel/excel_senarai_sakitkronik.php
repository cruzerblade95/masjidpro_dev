...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Senarai Ahli Mempunyai Sakit Kronik');
//setting Font Style
$styleFont = array(
      'font' => array(
            'bold'      => true,
...
$objPHPExcel->getActiveSheet()->setCellValue('F5', 'Penyakit');
$objPHPExcel->getActiveSheet()->setCellValue('G5', 'Rawatan');
...
$sqll="select a.id_sakit,b.id_data,b.nama_penuh,b.no_ic,b.no_hp,a.jenis_penyakit,a.rawatan_terkini,a.id_masjid from sej6x_data_sakit a 
      inner join sej6x_data_peribadi b on a.no_kp=b.no_ic
      where a.id_masjid='$id_masjid'
      union ALL
      select c.id_sakit,c.id_data,d.nama_penuh,d.no_ic,d.no_tel as no_hp,c.jenis_penyakit,c.rawatan_terkini,c.id_masjid from sej6x_data_sakit c 
...
while($listl = mysql_fetch_assoc($sqlqueryl)){
   ...
   $objPHPExcel->getActiveSheet()->setCellValue("F$noindex", $listl['jenis_penyakit']);
   $objPHPExcel->getActiveSheet()->setCellValue("G$noindex", $listl['rawatan_terkini']);
...
$objWriter->save('output/'.$kod_masjid.'/LaporanSenaraiPenyakitKronik.xlsx');
