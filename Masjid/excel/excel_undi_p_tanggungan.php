...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Senarai Tanggungan Perempuan Layak Mengundi');
//setting Font Style
$styleFont = array(
	'font' => array(
		'bold'      => true,
...
$objPHPExcel->getActiveSheet()->getStyle('B5:G5')->getAlignment()->setWrapText(true);
...
//$currentYear= get_setting();
	...
	$sqll="SELECT * FROM sej6x_data_anakqariah WHERE id_masjid='$id_masjid' AND jantina='2' AND status_undi='1'";
...
while($listl = mysql_fetch_assoc($sqlqueryl)){
		...
		$objPHPExcel->getActiveSheet()->setCellValue("E$noindex", $listl['tarikh_lahir']);
		$objPHPExcel->getActiveSheet()->setCellValue("F$noindex", 'Perempuan');
		$objPHPExcel->getActiveSheet()->setCellValue("G$noindex", $listl['no_tel']);
...
//alignment list
$tindex = $cnt + 5;
$objPHPExcel->getActiveSheet()->getStyle("B6:G$tindex")->applyFromArray(
		array(
			'font'    => array(
				'name'		=> 'arial',
				'size'		=> '8'
...
$objPHPExcel->getActiveSheet()->getStyle("B6:G$tindex")->getAlignment()->setWrapText(true);
...
$objWriter->save('output/'.$kod_masjid.'/SenaraiLayakMengundiPerempuanTanggungan.xlsx');
