...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'Senarai Ahli Kariah Lelaki Layak Mengundi');
//setting Font Style
$styleFont = array(
	'font' => array(
		'bold'      => true,
...
//$currentYear= get_setting();
	...
	$sqll="SELECT * FROM sej6x_data_peribadi WHERE id_masjid='$id_masjid' AND jantina='1' AND data_undi='1'";
...
while($listl = mysql_fetch_assoc($sqlqueryl)){
		...
		$objPHPExcel->getActiveSheet()->setCellValue("G$noindex", $listl['no_hp']);
		$objPHPExcel->getActiveSheet()->setCellValue("H$noindex", $listl['alamat_terkini']);
...
$objWriter->save('output/'.$kod_masjid.'/SenaraiLayakMengundiLelaki.xlsx');
