...
if($id_surat != NULL) $q_surat = "UPDATE surat_rasmi SET id_masjid = ?, jenis_surat = ?, no_rujukan = ?, tajuk_surat = ?, tarikh = ?, tarikh_majlis = ?, masa = ?, masa_tamat = ?, penerima = ?, alamat_1 = ?, alamat_2 = ?, alamat_3 = ?, poskod = ?, bandar = ?, id_negeri = ?, last_added = ? WHERE id_surat = ?";
...
if($id_surat != NULL) $stmt->bind_param("iiisssssssssssisi", $id_masjid, $jenis_surat, $no_rujukan, $tajuk_surat, $tarikh, $tarikh_majlis, $masa, $masa_tamat, $penerima, $alamat_1, $alamat_2, $alamat_3, $poskod, $bandar, $id_negeri, $date_now, $id_surat);
if($stmt->execute()) {
