<?php
    ...
    $query_list_nama = "SELECT a.id_dataajk 'id_dataajk', a.jawatan 'jawatan', b.nama_penuh 'nama_penuh', a.rank 'rank' FROM data_ajkmasjid a, sej6x_data_peribadi b WHERE a.id_masjid = $id_masjid AND a.id_ajk = b.id_data 
                        UNION
    ...
    $list_nama1 = mysqli_query($bd2, $query_list_nama) or die(mysqli_error($bd2));
    ...
    $row_list_nama1 = mysqli_fetch_assoc($list_nama1);
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        $disemak = $_POST['disemak'];
        $disahkan = $_POST['disahkan'];
        ...
        $q_meeting = "INSERT INTO minit_mesyuarat 
        (id_masjid, tajuk, rujukan_auto, no_rujukan, no_rujukan_manual, tahun, tarikh, masa, masa_tamat, tempat, id_disediakan, id_disemak, id_disahkan, last_added) 
        VALUES ($id_masjid, '$tajuk', $rujukan_auto, '$no_rujukan', '$no_rujukan_manual', '$tahun', '$tarikh', '$masa', '$masa_tamat', '$tempat', '$disediakan', '$disemak', '$disahkan', '$date_now')";
        }

        $insert_meeting = mysqli_query($bd2, $q_meeting) or die(mysqli_error($bd2));
            ...
            if($ketua_array[$a] > 0) {
                ...
                for ($i = 0; $i < $ketua_array[$a]; $i++) {
                    ...
                    if($id_kehadiran == NULL) {
                        $q_hadir = "INSERT INTO kehadiran_mesyuarat 
                    (id_masjid, id_mesyuarat, id_ajk, nama, jawatan, jenis_kehadiran) 
                    VALUES ($id_masjid, '$id_mesyuarat', '$id_ajk', '$nama', '$jawatan', '$jenis_kehadiran')";
                    }

                    $insert_kehadiran = mysqli_query($bd2, $q_hadir) or die(mysqli_error($bd2));
        ...
        if($perkara > 0) {
            ...
            for ($l = 0; $l < $perkara; $l++) {
                ...
                $perkara_isu = $_POST['perkara_isu'][$l];
                ...
                if ($id_perkara == null) {
                    $q_perkara = "INSERT INTO perkara_mesyuarat 
                (id_masjid, id_mesyuarat, perkara_isu, status_tindakan) 
                VALUES ($id_masjid, '$id_mesyuarat', '$perkara_isu', '$status_tindakan')";
                }

                $insert_perkara = mysqli_query($bd2, $q_perkara) or die(mysqli_error($bd2));
								...
								<div class="form-group row">
									...
									<div class="col-12 col-md-6">
										...
										<select class="form-control" name="disemak" id="disemak" required>
											<option>Pilih Nama:-</option>
											<?php do { ?>
												<option value="<?php echo($row_list_nama1['id_dataajk']); ?>" <?php if($row_list_nama1['id_dataajk'] == $r1['id_disemak']) echo 'selected'; ?>><?php echo($row_list_nama1['nama_penuh']); ?></option>
											<?php } while($row_list_nama1 = mysqli_fetch_assoc($list_nama1)); ?>
										</select>
									</div>
								</div>
								<div class="form-group row">
									<div class="col-12 col-md-12"><div align="center">
											...
											<select class="form-control" name="disahkan" id="disahkan" required>
												<option>Pilih Nama:-</option>
												<?php do { ?>
													<option value="<?php echo($row_list_nama2['id_dataajk']); ?>" <?php if($row_list_nama2['id_dataajk'] == $r1['id_disahkan']) echo 'selected'; ?>><?php echo($row_list_nama2['nama_penuh']); ?></option>
						...
						<div class="row">
							...
							<div class="col-12">
								<div class="form-group row">
									...
									<div class="col-6 col-md-2"><strong>Jawatan</strong></div>
									<div class="col-3 col-md-2"><strong>Kehadiran</strong></div>
									<div class="col-3 col-md-2"><strong>Tidak Hadir Bersebab</strong></div>
								</div>
								<div class="form-group row">
									<?php $i = 1; do {
										if($id_mesyuarat != NULL) {
											$q_hadir_ada = "SELECT * FROM kehadiran_mesyuarat WHERE id_mesyuarat = $id_mesyuarat AND id_ajk = " . $row_list_nama3['id_dataajk'];
										...
										<div class="col-11 col-md-5"><input name="nama[]" type="hidden" value="<?php echo($row_list_nama3['nama_penuh']); ?>" /><?php echo($row_list_nama3['nama_penuh']); ?></div>
										<div class="col-6 col-md-2"><input name="jawatan[]" type="hidden" value="<?php echo($row_list_nama3['jawatan']); ?>" /><?php echo($row_list_nama3['jawatan']); ?></div>
										<div class="col-3 col-md-2" style="padding-left: 5%;"><input id="tanda_<?php echo($i); ?>" name="tanda_kehadiran[]" type="checkbox" onclick="checkboxTrigger(<?php echo $i ?>)" <?php if($id_mesyuarat != NULL && $r2_hadir['jenis_kehadiran'] == 1) echo 'value="1" checked'; else echo 'value="99"'; ?>><label for="tanda_<?php echo($i); ?>"></label></div>
										<div class="col-3 col-md-2" style="padding-left: 5%;"><input id="tanda_tak_hadir_<?php echo($i); ?>" name="tanda_kehadiran_tak_hadir[]" type="checkbox" onclick="checkboxTriggerTakHadir(<?php echo $i ?>)" <?php if($id_mesyuarat != NULL && $r2_hadir['jenis_kehadiran'] == 1) echo 'value="1" checked'; else echo 'value="99"'; ?>><label for="tanda_tak_hadir_<?php echo($i); ?>"></label></div>
										<input name="jenis_kehadiran[]" type="hidden" value="<?php if($id_mesyuarat != NULL && $r2_hadir['jenis_kehadiran'] == 1) echo '1'; else echo '99'; ?>"/>
										<input name="id_ajk[]" type="hidden" value="<?php echo($row_list_nama3['id_dataajk']); ?>" />
										<input name="id_kehadiran[]" type="hidden" value="<?php if($id_mesyuarat != NULL) echo $r2_hadir['id_kehadiran']; else echo ''; ?>" />
										<?php $i++; } while($row_list_nama3 = mysqli_fetch_assoc($list_nama3)); ?>
								</div>
							</div>
						</div>
...
<script>
	...
	function checkboxTrigger(a){
		var tanda = $("#tanda_"+a+"");
		$("#tanda_"+a+"").removeAttr("disabled");
		$("#tanda_tak_hadir_"+a+"").attr("disabled", true);
		console.log($("#tanda_"+a+"").checked);
	}
	function checkboxTriggerTakHadir(a){
		$("#tanda_"+a+"").attr("disabled", true);
		$("#tanda_tak_hadir_"+a+"").removeAttr("disabled");
	}
