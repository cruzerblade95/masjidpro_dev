...
if($_GET['markVoid'] == 1 && $_GET['id'] != NULL) {
    ...
    $bilNo = $idBill[1];
    if($typeJournalEntry == 2) $extraBill = "receivedNo = $bilNo";
    else if($typeJournalEntry == 1) $extraBill = "payNo = $bilNo";
...
if($pilihRekod != NULL && strpos($pilihRekod, 'A') !== false) {
    ...
    $group_by_namaan = "IF (a.payNo IS NOT NULL, a.payNo, IF (a.receivedNo IS NOT NULL, a.receivedNo, ''))";
}
else if($pilihRekod != NULL && $pilihRekod != 1) $extra = "AND a.typeRecords = $pilihRekod";
else {
    $pilihRekod = 'SEMUA';
    $group_by_namaan = "IF(a.payNo IS NULL AND a.receivedNo IS NULL, CONCAT(a.id, '-', FLOOR(RAND() * (99999-10000) + 10000)), CONCAT(a.typeJournalEntry, '-', IF(a.payNo IS NOT NULL, a.payNo, a.receivedNo)))";
};
...
$qBase = "SELECT IF(a.payNo IS NULL AND a.receivedNo IS NULL, CONCAT(a.typeRecords, '-', a.id, '-', FLOOR(RAND() * (99999-10000) + 10000)), CONCAT(a.typeJournalEntry, '-', IF(a.payNo IS NOT NULL, a.payNo, a.receivedNo))) 'Bil',
       a.dateRecords 'Tarikh', b.categoryName 'Akaun', a.vendor 'Nama', a.particulars 'Butiran', a.susut_nilai 'susut_nilai',
       ...
       a.voidStatus, a.typeRecords, a.typeJournalEntry, b.categoryType, b.assetType, a.accountsCategory_id, a.pairAccountsCategory_id, a.jenis_transaksi,
       a.payNo, a.receivedNo
FROM accountsRecords a LEFT JOIN accountsCategory b ON ((a.pairAccountsCategory_id = b.id AND a.jenis_transaksi IS NULL) OR (a.accountsCategory_id = b.id AND a.pairAccountsCategory_id IS NULL) OR (a.jenis_transaksi IS NOT NULL AND a.accountsCategory_id = b.id))
WHERE a.id_masjid = $id_masjid AND a.typeModule = $training $extra $extra2
GROUP BY $group_by_namaan
$susun";
// echo($qBase);
// exit;
