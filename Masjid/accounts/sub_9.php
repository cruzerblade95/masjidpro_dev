    ...
    <div class="col-12 col-md-12">
        <div class="card">
            <div class="card-header bg-info" align="center">
                <h5 class="m-b-0 text-white font-weight-bold">Rekod Transaksi Kewangan</h5>
            </div>
            <div class="card-body">
                            ...
                            <table style="width: 100%; color: black" id="meja_akaun_baru" data-order="[]" class="table table-bordered table-hover display margin-top-10 w-p100 table-striped">
                                ...
                                <tr>
                                    <?php foreach ($field_listRekod as $field) {
                                        $exceptColumn = array("id","voidStatus", "typeRecords", "typeJournalEntry", "Nama", "Butiran", "susut_nilai", "categoryType", "assetType", "accountsCategory_id", "pairAccountsCategory_id", "jenis_transaksi", "Bil", "Pair Account Category Name", "Account Category Name", "Sum Batch Amount");
                                ...
                                <?php $i = 1; $baki[0] = NULL; do {
                                    ...
                                    $hasBatch = $row_listRekod['hasBatch'];
                                    $query = $row_listRekod['query'];
                                    ...
                                    if(strpos($_GET['pilihRekod'], "A") == 0 && $typeJournalEntry == 1 && $typeRecords == 3 && $hasBatch > 1){
                                        $pendapatan = $row_listRekod['Perbelanjaan<br />(RM)'];
                                        $perbelanjaan = $row_listRekod['Pendapatan<br />(RM)'];
                                    }else if($typeJournalEntry == 1 && $typeRecords == 3 && $hasBatch > 1 && $pilihRekod == "SEMUA"){
                                        $pendapatan = $row_listRekod['Perbelanjaan<br />(RM)'];
                                        $perbelanjaan = $row_listRekod['Pendapatan<br />(RM)'];
                                    }
                                    ...
                                    // }
                                        ...
                                        $pendapatan = $row_listRekod['Pendapatan<br />(RM)'];
                                        $perbelanjaan = $row_listRekod['Perbelanjaan<br />(RM)'];
                                    }
                                    else{
                                        $pendapatan = $row_listRekod['Pendapatan<br />(RM)'];
                                        $perbelanjaan = $row_listRekod['Perbelanjaan<br />(RM)'];
                                    ...
                                    if($voidStatus != 1) 
                                    {
                                        ...
                                        if($pendapatan != NULL){
                                            $baki[$i] = $baki[$i-1] + $pendapatan;
                                        }else if($perbelanjaan != NULL){
                                            $baki[$i] = $baki[$i-1] - $perbelanjaan;
                                        }
                                        // $baki[$i] = $baki[$i-1] + $row_listRekod['Pendapatan<br />(RM)'] - $row_listRekod['Perbelanjaan<br />(RM)'];
                                    }
                                    else 
                                    {
                                        $baki[$i] = $baki[$i-1];
                                    ...
                                    <tr <?php echo $voidStatus == 1 ? 'style="text-decoration: line-through"' : NULL; ?>>
                                        ...
                                        <?php if($pilihSusun == 4 || $pilihSusun != 5) { ?><td style="width: 100px; vertical-align: middle"><div align="right"><?php echo number_format($pendapatan, 2); ?></div></td><?php } ?>
                                        <?php if($pilihSusun == 5 || $pilihSusun != 4) { ?><td style="width: 100px; vertical-align: middle"><div align="right"><?php echo number_format($perbelanjaan, 2); ?></div></td><?php } ?>
                                        <?php if($pilihRekod != 1 && $pilihSusun != 6 && $pilihRekod != 'SEMUA') { ?>
                                            <td style="width: 100px; vertical-align: middle"><div align="right">
                                            <?php 
                                                echo number_format($baki[$i], 2); 
                                            ?>
                                        </div></td>
                                        <?php } ?>
                                        <td style="vertical-align: middle; width: fit-content">
                                            <div align="center">
                                                <?php if($row_listRekod['typeRecords'] == 1 || $row_listRekod['typeRecords'] == 3) { ?>
                                                    ...
                                                    <?php if($voidStatus != 1) { ?>&nbsp;<button type="button" class="btn btn-danger fas fa-trash-alt" onClick="modal_delete_akaun('<?php echo $training == 1 ? 'Batalkan' : 'Padam'; ?> Rekod Pendapatan / Perbelanjaan', 'Akaun: <?php echo ($row_listRekod['Akaun']); ?><br />Nama: <?php echo ($row_listRekod['Nama']); ?><br />Butiran: <?php echo ($row_listRekod['Butiran']); ?><br /><?php echo $pendapatan != NULL ? 'Pendapatan (RM): '.$pendapatan : 'Perbelanjaan (RM): '.$perbelanjaan; ?>', '&subModul=9&markVoid=1&deleteId=<?php echo ($row_listRekod['id']); ?>')"></button><?php } ?>
                                                <?php } else { ?>
